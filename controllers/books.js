...
module.exports.getBooksInGenre = async (req, res) => {
    ...
    if (rating && !isNaN(rating) && !(rating > 5) && !(rating < 1)) {
        ...
        if (result.data.length > 0) {
            ...
            const err = new ExpressError(`Sorry we could not find any books in ${genre} with a rating in the range of ${rating}`, 404);
            return res.status(err.statusCode).json({ err });
        }
    }
...
module.exports.getBooksByAuthor = async (req, res) => {
    ...
    if (rating && !isNaN(rating) && !(rating > 5) && !(rating < 1)) {
        ...
        if (result.data.length > 0) {
            ...
            const err = new ExpressError(`Sorry we could not find any books rated by ${author} in the range of ${ratingVal}`, 404);
            return res.status(err.statusCode).json({ err });
        }
    }
